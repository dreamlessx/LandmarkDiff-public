...
def parse_slurm_log(log_path: str) -> TrainingMetrics:
    ...
    step_pattern = re.compile(
        r"step\s+(\d+).*?loss[=:]\s*([\d.e-]+)", re.IGNORECASE
    )
    lr_pattern = re.compile(r"lr[=:]\s*([\d.e-]+)", re.IGNORECASE)
    grad_pattern = re.compile(r"grad_norm[=:]\s*([\d.e-]+)", re.IGNORECASE)
    val_pattern = re.compile(
        r"val.*?(ssim|lpips|nme)[=:]\s*([\d.e-]+)", re.IGNORECASE
    )
    speed_pattern = re.compile(r"([\d.]+)\s*(?:it/s|steps/s|s/step)", re.IGNORECASE)
...
def detect_convergence_issues(metrics: TrainingMetrics) -> list[dict]:
    ...
    if window > 50:
        rolling_std = np.array([
            np.std(losses[max(0, i - window):i])
...
def check_phase_transition(metrics: TrainingMetrics, min_steps: int = 20000) -> dict:
    ...
    if total_steps < min_steps:
        result["reasons"].append(
            f"Only {total_steps} steps completed (minimum: {min_steps})"
        )
        result["recommendations"].append(f"Continue training for {min_steps - total_steps} more steps")
    else:
...
def generate_report(
    ...
    if metrics.losses:
        ...
        lines.append(f"| Best | {np.min(losses):.6f} (step {metrics.steps[int(np.argmin(losses))]}) |")
        lines.append(f"| Improvement | {(np.mean(losses[:100]) - np.mean(losses[-100:])) / np.mean(losses[:100]) * 100:.1f}% |")
    ...
    if metrics.diff_losses:
        lines.append("\n## Phase B Loss Components")
        lines.append(f"| Component | Mean | Final |")
        lines.append(f"|-----------|------|-------|")
        for name, vals in [
    ...
    if metrics.val_ssim:
        lines.append("\n## Validation Metrics")
        lines.append(f"| Step | SSIM | LPIPS | NME |")
        lines.append(f"|------|------|-------|-----|")
    ...
    if checkpoints:
        lines.append("\n## Checkpoints")
        lines.append(f"| Step | Size (MB) | EMA | Path |")
        lines.append(f"|------|-----------|-----|------|")
...
def main():
    ...
    parser.add_argument("--log", default=None,
                        help="Specific log file to parse (auto-detected if omitted)")
    parser.add_argument("--phase_check", action="store_true",
                        help="Check Phase A → B transition readiness")
    parser.add_argument("--min_steps", type=int, default=20000,
    ...
    parser.add_argument("--eval_val", action="store_true",
                        help="Evaluate checkpoints on validation set")
    ...
    if args.log:
        ...
        log_files = find_log_files(ckpt_dir)
